...
namespace System.CommandLine.Invocation
{
    public class ModelBindingCommandHandler : ICommandHandler
    {
        ...
        private Dictionary<IValueDescriptor,
            IValueSource> invokeArgumentBindingSources { get; } =
        ...
        public async Task<int> InvokeAsync(InvocationContext context)
        {
            ...
            var invocationArguments = boundValues.Select(x => x.Value)
                .ToArray();
            ...
            if (_handlerDelegate is null)
            {
                ...
                {
                    _invocationTargetBinder?.UpdateInstance(
                        invocationTarget,
                        bindingContext
                    );
                }

                invocationTarget ??= _invocationTargetBinder?.CreateInstance(
                    bindingContext
                );
        ...
        private void BindValueSource(
            ...
            {
                throw new InvalidOperationException(
                    "You must bind to a parameter on this handler"
                );
            }
            invokeArgumentBindingSources.Add(paramDesc, valueSource);
        }

        private ParameterDescriptor? FindParameterDescriptor(
            ParameterInfo? param
        ) =>
            param == null
                ? null
